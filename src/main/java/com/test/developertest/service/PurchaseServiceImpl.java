...
public class PurchaseServiceImpl implements PurchaseService {
    ...
    public List<String> nameOfHalfAYear() {
        ...
        for (Purchase purchase : daoPurchase.getAllPurchases()) {
            ...
            if (currenPeriod.getMonths() <= period.getMonths()) {
                if (result.containsKey(purchase.getName())) {
                    result.put(purchase.getName(), result.get(purchase.getName()) + 1);
                } else {
        ...
        List<String> result2 = result.entrySet().stream().filter(entry -> maxPurchases.equals(entry.getValue())).map(Map.Entry::getKey).collect(Collectors.toList());
